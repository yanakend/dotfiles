...
"==== Modify start {{{2
    call s:MRU_AddFile(bufnr(a:fname))
"==== Modify end }}}
...
"==== Modify start {{{2
    ...
    nnoremap <buffer> <silent> q :let @/=b:saved_mru_search<CR>:noh<CR>:close<CR>:wincmd p<CR>
    nnoremap <buffer> <silent> <ESC> :let @/=b:saved_mru_search<CR>:noh<CR>:close<CR>:wincmd p<CR>
    nnoremap <buffer> <silent> s :call <SID>SearchStrings()<CR>
    ...
    function! s:SearchStrings()
      let _key = input("Search for pattern : ")
      let _key = s:MRU_escape_filename(_key)
        let _key = substitute(_key, '\.', '\\.', 'g')
        let @/='^'._key
        setlocal modifiable
...
if !exists('g:MRU_hi_FnamePath')
  hi link MruFnamePath  WarningMsg
endif
